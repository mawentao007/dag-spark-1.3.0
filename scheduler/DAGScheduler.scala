...
class DAGScheduler(
  ...
  //marvin v2
  val stageIdToHeight = new HashMap[Int,Int]
  private def calculateDAG(stage:Stage) {
    val missing = getParentStages(stage.rdd, stage.jobId)
    if (missing != Nil) {
      for (st <- missing) {
        if(stageIdToHeight.getOrElse(st.id,-1) <= stageIdToHeight(stage.id)) {
          stageIdToHeight(st.id) = stageIdToHeight(stage.id) + 1
        }
        calculateDAG(st)
      }
    }
      ...
      properties: Properties = null)
  {
    ...
    if (finalStage != null) {
      ...
      if (shouldRunLocally) {
        ...
        //marvin v2
        stageIdToHeight(finalStage.id) = 0
        calculateDAG(finalStage)
        for(st <- stageIdToHeight){
          logInfo("%%%%%% stage Id " + st._1 + " weight " + st._2 + " %%%%%%")
        }
  ...
  private def submitStage(stage: Stage) {
    ...
    //marvin
    val partStageIdToHeight = new HashMap[Int,Int]
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (missing == Nil) {
          ...
          //marvin v2
          for(parent <- missing){
            partStageIdToHeight(parent.id) = stageIdToHeight(parent.id)
          }
          //对所依赖的父stage根据height进行排序，取最高的执行（sortBy默认是升序）
          for(st <- partStageIdToHeight.toList.sortBy(_._2).reverse){
            submitStage(stageIdToStage(st._1))
          }
          waitingStages += stage
        }
      }
